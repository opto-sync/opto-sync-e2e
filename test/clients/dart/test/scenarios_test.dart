...
Future<void> main() async {
  ...
  Future<HttpResult> flushOne(
      OptoSyncClient client, Mutation mutation, String id) async {
    final res =
        await syncDoc(id, jsonDecode(mutation.jsonPayload) as Map<String, dynamic>);
    await markMutation(
        client, mutation.id, res.ok ? SyncStatus.synced : SyncStatus.failed);
    return res;
  }

  group('opto_sync_client client-in-the-loop e2e', () {
    setUpAll(() {
      syncer = newSyncer();
    });
    ...
    test('1a. offline queue flushed one-by-one: all synced, server has every '
      ...
      expect((await pendingMutations(client)).length, mutations.length,
          reason: 'all mutations must be queued as pending');
      expect(await statusCounts(client), const StatusCounts(3, 3, 0, 0),
          reason: 'nothing may be marked synced before a flush');
      expect((await getDocData(id))['m1'], isNull,
          reason: 'server must be untouched while offline');

      // Back online: flush in queue order.
      for (final m in await pendingMutations(client)) {
        final res = await flushOne(client, m, id);
        expect(res.status, 200, reason: 'sync of mutation ${m.id}: ${res.body}');
        expect((res.json as Map)['merged'], isTrue);
        expect((res.json as Map)['mergedWith'], 'native-c-ffi',
            reason: 'the server must merge with the C core');
      }

      expect(await pendingMutations(client), isEmpty,
          reason: 'queue must be drained');
      expect(await statusCounts(client), const StatusCounts(3, 0, 3, 0));

      expectDeepEqual(await getDocData(id), fx['expected'],
          'server document after individual flush');
    });

    test('1b. offline queue flushed atomically via /sync/batch: same result',
        () async {
      final fx = scenario('offlineQueue');
      ...
      expectDeepEqual(
          await getDocData(id), fx['expected'], 'server document after batch flush');
    });
    ...
    test('2. optimistic local write then server pull-back reconcile converges',
        () async {
      ...
      final localAfterOptimistic = await client.reconcileIncoming(
          'docs', id, obj(fx, 'mutation'), obj(fx, 'base'));
      expectDeepEqual(localAfterOptimistic, fx['expected'],
      ...
      expectDeepEqual(serverData, fx['expected'], 'server document after push');

      // Reconcile the pulled server state back into the local copy.
      final localAfterPullback = await client.reconcileIncoming(
          'docs', id, serverData, localAfterOptimistic);
      expectDeepEqual(
          localAfterPullback, serverData, 'local copy after pull-back vs server');
      expectDeepEqual(localAfterPullback, fx['expected'],
      ...
      expectDeepEqual(jsonDecode(await getDocRaw(id)), fx['expected'],
    ...
    test('3. stale server state loses to newer local; fresher server state wins',
        () async {
      ...
      expectDeepEqual(stale, fx['serverStale'], 'server precondition (stale)');

      final survived =
          await client.reconcileIncoming('docs', id, stale, obj(fx, 'local'));
      expectDeepEqual(survived, fx['expectedLocalSurvives'],
          'local value must survive a stale server pull');
      expect(survived['sOnly'], isNull,
          reason: 'whole-object rejection: no key from the stale doc may leak in');

      // Now the server holds a NEWER state.
      await putDoc(id, fx['serverFresh']!);
      final fresh = await getDocData(id);
      final overwritten =
          await client.reconcileIncoming('docs', id, fresh, obj(fx, 'local'));
      expectDeepEqual(overwritten, fx['expectedServerWins'],
          'fresher server state must win');
      expect(overwritten['lOnly'], 'local-marker',
    ...
    test('4. keyed array: untouched kept, fresh applied, stale rejected, new '
      ...
      final localCopy = await client.reconcileIncoming(
          'docs', id, obj(fx, 'mutation'), obj(fx, 'base'));
      expectDeepEqual(localCopy, fx['expected'], 'client-side keyed-array reconcile');
      ...
      final serverData = await getDocData(id);
      expectDeepEqual(serverData, fx['expected'], 'server keyed-array merge');

      final rows = (serverData['rows'] as List).cast<Map<String, dynamic>>();
      expect(rows.length, 4, reason: 'exactly one new identity may be appended');
      expect(rows.where((r) => r['id'] == 'r4').length, 1,
          reason: 'r4 must not be duplicated');
      expect(rows[3]['id'], 'r4',
          reason: 'a new identity is appended at the END of the base array');
      expect(rows.firstWhere((r) => r['id'] == 'r3')['label'], 'server-fresh',
          reason: 'the stale element must not be applied');
      expect(rows.firstWhere((r) => r['id'] == 'r1')['label'], 'untouched',
    ...
    test('5. replaying the same queued mutation leaves the document '
      ...
      expectDeepEqual(afterFirst['data'], fx['expected'],
      ...
      expect(afterSecond['version'] as int, greaterThan(afterFirst['version'] as int),
          reason: 'the replay must really have written');
      expectDeepEqual(afterSecond['data'], afterFirst['data'],
          'replay must not change the document');
      expectDeepEqual(afterSecond['data'], fx['expected'], 'document after replay');
      expect(((afterSecond['data'] as Map)['tags'] as List).length, 2,
          reason: 'identity-less array elements must not duplicate on replay');
      expect(((afterSecond['data'] as Map)['rows'] as List).length, 2,
    ...
    test('6. a mutation against a nonexistent document is marked failed, not '
      ...
      final client = freshClient();
      await client.queueMutation('docs', missingId, obj(fx, 'mutationDoomed'));
      ...
      expect(await statusCounts(client), const StatusCounts(1, 0, 0, 1),
          reason: 'the failed mutation must be FAILED and must not count as '
              'pending or synced');
      ...
      expect(await statusCounts(client), const StatusCounts(2, 1, 0, 1),
      ...
      expect(await statusCounts(client), const StatusCounts(2, 0, 1, 1),
          reason: 'final accounting: one synced, one failed, nothing pending');
      expectDeepEqual(await getDocData(okId), fx['expectedOk'],
